...
from src.agents.base import AgentReturnType, BaseAgent, LLMType
...
class TaskSeperatorAgent(BaseAgent):
    ...
    def run(self) -> AgentReturnType:
        ...
        self.llm = cast(Groq, self.llm)

        logger.info("Running the agent :: TaskSeparator")
        ...
        self.outputs.append({"message": output.model_dump_json()})
        logger.info(
            f"Output from the TaskSeperator Agent: {json.dumps(output.model_dump_json(), indent=2)}"
        )
